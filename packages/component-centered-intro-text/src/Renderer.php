<?php
/**
 * Renderer — delegates to the Blade view for Centered Intro Text.
 *
...
namespace BalefireInc\Sage\CenteredIntroText;
...
class Renderer {
	...
	public static function render( array $props, string $wrapper_attributes = '' ): string {
		...
		if ( function_exists( '\Roots\view' ) ) {
			return \Roots\view( 'bma::components.centered-intro-text', $props )->render();
		}

		// Acorn installed standalone.
		if ( function_exists( '\Acorn\view' ) ) {
			return \Acorn\view( 'bma::components.centered-intro-text', $props )->render();
		}

		// No Blade runtime available — fail loud, not silent.
		return '<!-- balefire/centered-intro-text: Sage/Acorn Blade runtime not found. '
