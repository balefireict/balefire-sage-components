<?php
/**
 * balefireict/component-split-feature — bootstrap.
 *
 ...
 * Register the block.
 */
$bma_split_feature_boot = static function (): void {
	...
	if ( is_admin() && function_exists( 'wp_register_script' ) ) {
		...
		$editor_js = file_get_contents( __DIR__ . '/../blocks/split-feature/editor.js' );
		if ( $editor_js !== false ) {
			wp_add_inline_script( 'balefire-split-feature-editor', $editor_js );
		}
	}
...
if ( function_exists( 'add_action' ) ) {
	if ( did_action( 'init' ) ) {
		$bma_split_feature_boot();
	} else {
		add_action( 'init', $bma_split_feature_boot, 20 );
	}
} else {
	...
	$GLOBALS['wp_filter']['init'][20][] = [
		'function'      => $bma_split_feature_boot,
		'accepted_args' => 1,
	];
}

unset( $bma_split_feature_boot );
