<?php
/**
 * Renderer — delegates to the Blade view for Team Member (Centered).
 *
...
namespace BalefireInc\Sage\TeamMemberCentered;
...
class Renderer {
	...
	public static function render( array $props, string $wrapper_attributes = '' ): string {
		...
		if ( function_exists( '\Roots\view' ) ) {
			return \Roots\view( 'bma::components.team-member-centered', $props )->render();
		}

		// Acorn installed standalone.
		if ( function_exists( '\Acorn\view' ) ) {
			return \Acorn\view( 'bma::components.team-member-centered', $props )->render();
		}

		// No Blade runtime available — fail loud, not silent.
		return '<!-- balefire/team-member-centered: Sage/Acorn Blade runtime not found. '
