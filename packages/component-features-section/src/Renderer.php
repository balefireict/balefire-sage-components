<?php
/**
 * Renderer — delegates to the Blade view for Features Section.
 *
...
namespace BalefireInc\Sage\FeaturesSection;
...
class Renderer {
	...
	public static function render( array $props, string $wrapper_attributes = '' ): string {
		...
		if ( function_exists( '\Roots\view' ) ) {
			return \Roots\view( 'bma::components.features-section', $props )->render();
		}

		// Acorn installed standalone.
		if ( function_exists( '\Acorn\view' ) ) {
			return \Acorn\view( 'bma::components.features-section', $props )->render();
		}

		// No Blade runtime available — fail loud, not silent.
		return '<!-- balefire/features-section: Sage/Acorn Blade runtime not found. '
