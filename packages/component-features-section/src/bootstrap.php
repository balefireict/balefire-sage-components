...
$bma_features_section_boot = static function (): void {
	...
	if ( ! \BalefireInc\Sage\Support\Settings::isBlockEnabled( 'features-section' ) ) {
		return;
	}
	...
	if ( is_admin() && function_exists( 'wp_register_script' ) ) {
		wp_register_script(
			'balefire-features-section-editor',
			false,
		...
		$editor_js = file_get_contents( __DIR__ . '/../blocks/features-section/editor.js' );
		if ( $editor_js !== false ) {
			wp_add_inline_script( 'balefire-features-section-editor', $editor_js );
		}
	}
	...
	if ( function_exists( 'wp_register_style' ) ) {
		wp_register_style( 'balefire-features-section', false, [], null );

		$style_css = file_get_contents( __DIR__ . '/../blocks/features-section/style.css' );
		if ( $style_css !== false ) {
			wp_add_inline_style( 'balefire-features-section', $style_css );
		}
	}
...
if ( function_exists( 'add_action' ) ) {
	...
	$GLOBALS['wp_filter']['init'][20][] = [
		'function'      => $bma_features_section_boot,
		'accepted_args' => 1,
	];
}
