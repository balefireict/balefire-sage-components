...
// Fixed feature set carried from the source block; attributes control the
// heading, intro, tone and width only.
$features = [
    [
        'title'       => 'Marketing',
        'description' => 'Plan it, create it, launch it. Collaborate seamlessly with the whole organization and hit your marketing goals every month with a structured plan.',
        'icon'        => '<path fill-rule="evenodd" d="M3 3a1 1 0 000 2v8a2 2 0 002 2h2.586l-1.293 1.293a1 1 0 101.414 1.414L10 15.414l2.293 2.293a1 1 0 001.414-1.414L12.414 15H15a2 2 0 002-2V5a1 1 0 100-2H3zm11.707 4.707a1 1 0 00-1.414-1.414L10 9.586 8.707 8.293a1 1 0 00-1.414 0l-2 2a1 1 0 101.414 1.414L8 10.414l1.293 1.293a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path>',
    ],
    [
        'title'       => 'Legal',
        'description' => 'Protect your organization, devices, and compliance requirements with structured workflows and custom permissions made for your team.',
    ...
    [
        ...
        'description' => 'Auto-assign tasks, send notifications, and streamline operations with templates that help your team get moving faster.',
    ...
    [
        'title'       => 'Finance',
        'description' => 'Use audit-friendly workflows built for critical financial operations like month-end close, planning, and quarterly budgeting.',
    ...
    [
        ...
        'description' => 'Craft polished experiences for marketing and product with real cross-company collaboration and a clearer design system.',
    ...
    [
        'title'       => 'Operations',
        'description' => 'Keep your company moving with customizable, iterative, and structured workflows built for efficient teams and individuals.',
...
<section {{ $attributes->class(['bma-features-section', 'py-8 sm:py-16', $surface['section']]) }}>
    <div class="mx-auto w-full" style="{{ $innerStyle }}">
        <div class="mb-8 max-w-screen-md lg:mb-16">
            @if ($heading !== '')
                <h2 class="mb-4 text-4xl font-extrabold tracking-tight {{ $surface['heading'] }}">
                    {{ $heading }}
                </h2>
            @endif

            @if ($intro !== '')
                <p class="{{ $surface['body'] }} sm:text-xl">
                    {{ $intro }}
                </p>
            @endif
        </div>
        ...
        <div class="space-y-8 md:grid md:grid-cols-2 lg:grid-cols-3 md:gap-12 md:space-y-0">
            @foreach ($features as $feature)
                <div>
                    <div class="flex items-center justify-center w-10 h-10 mb-4 rounded-full lg:h-12 lg:w-12 {{ $surface['badge'] }}">
                        <svg class="w-5 h-5 lg:w-6 lg:h-6 {{ $surface['accent'] }}" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
                            {!! wp_kses($feature['icon'], $allowedSvg) !!}
                        </svg>
                    </div>
                    <h3 class="mb-2 text-xl font-bold {{ $surface['heading'] }}">
                        {{ $feature['title'] }}
                    </h3>
