<?php
/**
 * balefireict/component-prose-section — bootstrap.
 *
 * Registers the Gutenberg block. The block uses a PHP render callback that
 * delegates to a Blade view. No shortcode layer: InnerBlocks-based content
 * can't be expressed as shortcode attributes.
 *
 ...
 * Register the block.
 */
$bma_prose_section_boot = static function (): void {
	...
	if ( is_admin() && function_exists( 'wp_register_script' ) ) {
		...
		$editor_js = file_get_contents( __DIR__ . '/../blocks/prose-section/editor.js' );
		if ( $editor_js !== false ) {
			wp_add_inline_script( 'balefire-prose-section-editor', $editor_js );
		}
	}

	// --- Gutenberg block ---------------------------------------------------
	if ( function_exists( 'register_block_type' ) ) {
		register_block_type( __DIR__ . '/../blocks/prose-section' );
	}
};

if ( function_exists( 'add_action' ) ) {
	if ( did_action( 'init' ) ) {
		$bma_prose_section_boot();
	} else {
		add_action( 'init', $bma_prose_section_boot, 20 );
	}
} else {
	...
	$GLOBALS['wp_filter']['init'][20][] = [
		'function'      => $bma_prose_section_boot,
		'accepted_args' => 1,
	];
}

unset( $bma_prose_section_boot );
