<?php
/**
 * Renderer — delegates to the Blade view for Numbered Features.
 *
...
class Renderer {
	...
	public static function render( array $props, string $wrapper_attributes = '' ): string {
		...
		if ( function_exists( '\Roots\view' ) ) {
			return \Roots\view( 'bma::components.numbered-features', $props )->render();
		}

		// Acorn installed standalone.
		if ( function_exists( '\Acorn\view' ) ) {
			return \Acorn\view( 'bma::components.numbered-features', $props )->render();
		}

		// No Blade runtime available — fail loud, not silent.
		return '<!-- balefire/numbered-features: Sage/Acorn Blade runtime not found. '
