<?php
/**
 * Renderer — delegates to the Blade view for Preheader & Title.
 *
...
namespace BalefireInc\Sage\PreheaderAndTitle;
...
class Renderer {
	...
	public static function render( array $props, string $wrapper_attributes = '' ): string {
		...
		if ( function_exists( '\Roots\view' ) ) {
			return \Roots\view( 'bma::components.preheader-and-title', $props )->render();
		}

		// Acorn installed standalone.
		if ( function_exists( '\Acorn\view' ) ) {
			return \Acorn\view( 'bma::components.preheader-and-title', $props )->render();
		}

		// No Blade runtime available — fail loud, not silent.
		return '<!-- balefire/preheader-and-title: Sage/Acorn Blade runtime not found. '
