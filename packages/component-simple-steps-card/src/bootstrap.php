<?php
/**
 * balefireict/component-simple-steps-card — bootstrap.
 *
 ...
 * Register the block.
 */
$bma_simple_steps_card_boot = static function (): void {

	// Honor the wp-admin "Balefire Blocks" toggle for this block.
	if ( ! \BalefireInc\Sage\Support\Settings::isBlockEnabled( 'simple-steps-card' ) ) {
		return;
	}
	...
	if ( is_admin() && function_exists( 'wp_register_script' ) ) {
		wp_register_script(
			'balefire-simple-steps-card-editor',
			false,
		...
		$editor_js = file_get_contents( __DIR__ . '/../blocks/simple-steps-card/editor.js' );
		if ( $editor_js !== false ) {
			wp_add_inline_script( 'balefire-simple-steps-card-editor', $editor_js );
		}
	}

	// --- Gutenberg block ---------------------------------------------------
	if ( function_exists( 'register_block_type' ) ) {
		register_block_type( __DIR__ . '/../blocks/simple-steps-card' );
	}
};

if ( function_exists( 'add_action' ) ) {
	if ( did_action( 'init' ) ) {
		$bma_simple_steps_card_boot();
	} else {
		add_action( 'init', $bma_simple_steps_card_boot, 20 );
	}
} else {
	...
	$GLOBALS['wp_filter']['init'][20][] = [
		'function'      => $bma_simple_steps_card_boot,
		'accepted_args' => 1,
	];
}

unset( $bma_simple_steps_card_boot );
