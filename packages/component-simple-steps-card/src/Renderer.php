<?php
/**
 * Renderer — delegates to the Blade view for Simple Steps Card.
 *
...
namespace BalefireInc\Sage\SimpleStepsCard;
...
class Renderer {
	...
	public static function render( array $props, string $wrapper_attributes = '' ): string {
		...
		if ( function_exists( '\Roots\view' ) ) {
			return \Roots\view( 'bma::components.simple-steps-card', $props )->render();
		}

		// Acorn installed standalone.
		if ( function_exists( '\Acorn\view' ) ) {
			return \Acorn\view( 'bma::components.simple-steps-card', $props )->render();
		}

		// No Blade runtime available — fail loud, not silent.
		return '<!-- balefire/simple-steps-card: Sage/Acorn Blade runtime not found. '
