<?php
/**
 * balefireict/component-container — bootstrap.
 *
 ...
 * Register the block.
 */
$bma_container_boot = static function (): void {
	...
	if ( is_admin() && function_exists( 'wp_register_script' ) ) {
		...
		$editor_js = file_get_contents( __DIR__ . '/../blocks/container/editor.js' );
		if ( $editor_js !== false ) {
			wp_add_inline_script( 'balefire-container-editor', $editor_js );
		}
	}
...
if ( function_exists( 'add_action' ) ) {
	if ( did_action( 'init' ) ) {
		...
		add_action( 'init', $bma_container_boot, 20 );
	}
} else {
	...
	$GLOBALS['wp_filter']['init'][20][] = [
		'function'      => $bma_container_boot,
		'accepted_args' => 1,
	];
}

unset( $bma_container_boot );
