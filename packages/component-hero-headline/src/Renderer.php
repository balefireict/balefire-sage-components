<?php
/**
 * Renderer — delegates to the Blade view for Hero Headline.
 *
...
namespace BalefireInc\Sage\HeroHeadline;
...
class Renderer {
	...
	public static function render( array $props, string $wrapper_attributes = '' ): string {
		...
		if ( function_exists( '\Roots\view' ) ) {
			return \Roots\view( 'bma::components.hero-headline', $props )->render();
		}

		// Acorn installed standalone.
		if ( function_exists( '\Acorn\view' ) ) {
			return \Acorn\view( 'bma::components.hero-headline', $props )->render();
		}

		// No Blade runtime available — fail loud, not silent.
		return '<!-- balefire/hero-headline: Sage/Acorn Blade runtime not found. '
