<?php
/**
 * balefireict/component-image-text-row — bootstrap.
 *
 ...
 * Register the block.
 */
$bma_image_text_row_boot = static function (): void {

	// Honor the wp-admin "Balefire Blocks" toggle for this block.
	if ( ! \BalefireInc\Sage\Support\Settings::isBlockEnabled( 'image-text-row' ) ) {
		return;
	}
	...
	if ( is_admin() && function_exists( 'wp_register_script' ) ) {
		wp_register_script(
			'balefire-image-text-row-editor',
			false,
		...
		$editor_js = file_get_contents( __DIR__ . '/../blocks/image-text-row/editor.js' );
		if ( $editor_js !== false ) {
			wp_add_inline_script( 'balefire-image-text-row-editor', $editor_js );
		}
	}
	...
	if ( function_exists( 'wp_register_style' ) ) {
		wp_register_style( 'balefire-image-text-row', false, [], null );

		$style_css = file_get_contents( __DIR__ . '/../blocks/image-text-row/style.css' );
		if ( $style_css !== false ) {
			wp_add_inline_style( 'balefire-image-text-row', $style_css );
		}
	}

	// --- Gutenberg block ---------------------------------------------------
	if ( function_exists( 'register_block_type' ) ) {
		register_block_type( __DIR__ . '/../blocks/image-text-row' );
	}
};

if ( function_exists( 'add_action' ) ) {
	if ( did_action( 'init' ) ) {
		$bma_image_text_row_boot();
	} else {
		add_action( 'init', $bma_image_text_row_boot, 20 );
	}
} else {
	...
	$GLOBALS['wp_filter']['init'][20][] = [
		'function'      => $bma_image_text_row_boot,
		'accepted_args' => 1,
	];
}

unset( $bma_image_text_row_boot );
