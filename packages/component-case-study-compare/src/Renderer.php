<?php
/**
 * Renderer — delegates to the Blade view for Case Study Compare.
 *
...
namespace BalefireInc\Sage\CaseStudyCompare;
...
class Renderer {
	...
	public static function render( array $props, string $wrapper_attributes = '' ): string {
		...
		if ( function_exists( '\Roots\view' ) ) {
			return \Roots\view( 'bma::components.case-study-compare', $props )->render();
		}

		// Acorn installed standalone.
		if ( function_exists( '\Acorn\view' ) ) {
			return \Acorn\view( 'bma::components.case-study-compare', $props )->render();
		}

		// No Blade runtime available — fail loud, not silent.
		return '<!-- balefire/case-study-compare: Sage/Acorn Blade runtime not found. '
