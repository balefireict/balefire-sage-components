<?php
/**
 * Renderer — delegates to the Blade view for Card Stat.
 *
...
namespace BalefireInc\Sage\CardStat;
...
class Renderer {
	...
	public static function render( array $props, string $wrapper_attributes = '' ): string {
		...
		if ( function_exists( '\Roots\view' ) ) {
			return \Roots\view( 'bma::components.card-stat', $props )->render();
		}

		// Acorn installed standalone.
		if ( function_exists( '\Acorn\view' ) ) {
			return \Acorn\view( 'bma::components.card-stat', $props )->render();
		}

		// No Blade runtime available — fail loud, not silent.
		return '<!-- balefire/card-stat: Sage/Acorn Blade runtime not found. '
