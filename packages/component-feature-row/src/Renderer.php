<?php
/**
 * Renderer — delegates to the Blade view for Feature Row.
 *
...
namespace BalefireInc\Sage\FeatureRow;
...
class Renderer {
	...
	public static function render( array $props, string $wrapper_attributes = '' ): string {
		...
		if ( function_exists( '\Roots\view' ) ) {
			return \Roots\view( 'bma::components.feature-row', $props )->render();
		}

		// Acorn installed standalone.
		if ( function_exists( '\Acorn\view' ) ) {
			return \Acorn\view( 'bma::components.feature-row', $props )->render();
		}

		// No Blade runtime available — fail loud, not silent.
		return '<!-- balefire/feature-row: Sage/Acorn Blade runtime not found. '
