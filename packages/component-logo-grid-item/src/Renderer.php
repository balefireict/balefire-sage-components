<?php
/**
 * Renderer — delegates to the Blade view for Logo Grid Item.
 *
...
namespace BalefireInc\Sage\LogoGridItem;
...
class Renderer {
	...
	public static function render( array $props, string $wrapper_attributes = '' ): string {
		...
		if ( function_exists( '\Roots\view' ) ) {
			return \Roots\view( 'bma::components.logo-grid-item', $props )->render();
		}

		// Acorn installed standalone.
		if ( function_exists( '\Acorn\view' ) ) {
			return \Acorn\view( 'bma::components.logo-grid-item', $props )->render();
		}

		// No Blade runtime available — fail loud, not silent.
		return '<!-- balefire/logo-grid-item: Sage/Acorn Blade runtime not found. '
