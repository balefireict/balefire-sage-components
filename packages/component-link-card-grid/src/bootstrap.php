<?php
/**
 * balefireict/component-link-card-grid — bootstrap.
 *
 * Registers the Gutenberg block. The block uses a PHP render callback that
 * delegates to a Blade view. No shortcode layer: the items attribute is an
 * array, which shortcodes can't express.
 *
 ...
 * Register the block.
 */
$bma_link_card_grid_boot = static function (): void {
	...
	if ( is_admin() && function_exists( 'wp_register_script' ) ) {
		wp_register_script(
			'balefire-link-card-grid-editor',
			false,
		...
		$editor_js = file_get_contents( __DIR__ . '/../blocks/link-card-grid/editor.js' );
		if ( $editor_js !== false ) {
			wp_add_inline_script( 'balefire-link-card-grid-editor', $editor_js );
		}
	}
...
if ( function_exists( 'add_action' ) ) {
	if ( did_action( 'init' ) ) {
		$bma_link_card_grid_boot();
	} else {
		add_action( 'init', $bma_link_card_grid_boot, 20 );
	}
} else {
	...
	$GLOBALS['wp_filter']['init'][20][] = [
		'function'      => $bma_link_card_grid_boot,
		'accepted_args' => 1,
	];
}

unset( $bma_link_card_grid_boot );
