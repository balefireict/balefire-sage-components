<?php
/**
 * Renderer — delegates to the Blade view for Product Switcher.
 *
...
class Renderer {
	...
	public static function render( array $props, string $wrapper_attributes = '' ): string {
		...
		if ( function_exists( '\Roots\view' ) ) {
			return \Roots\view( 'bma::components.product-switcher', $props )->render();
		}

		// Acorn installed standalone.
		if ( function_exists( '\Acorn\view' ) ) {
			return \Acorn\view( 'bma::components.product-switcher', $props )->render();
		}

		// No Blade runtime available — fail loud, not silent.
		return '<!-- balefire/product-switcher: Sage/Acorn Blade runtime not found. '
