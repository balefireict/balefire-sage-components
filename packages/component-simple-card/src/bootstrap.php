 ...
 * Register the block.
 */
$bma_simple_card_boot = static function (): void {

	// Honor the wp-admin "Balefire Blocks" toggle for this block.
	if ( ! \BalefireInc\Sage\Support\Settings::isBlockEnabled( 'simple-card' ) ) {
		return;
	}
	...
	if ( is_admin() && function_exists( 'wp_register_script' ) ) {
		...
		$editor_js = file_get_contents( __DIR__ . '/../blocks/simple-card/editor.js' );
		if ( $editor_js !== false ) {
			wp_add_inline_script( 'balefire-simple-card-editor', $editor_js );
		}
	}

	// --- Gutenberg block ---------------------------------------------------
	if ( function_exists( 'register_block_type' ) ) {
		register_block_type( __DIR__ . '/../blocks/simple-card' );
	}
};
...
if ( function_exists( 'add_action' ) ) {
	if ( did_action( 'init' ) ) {
		...
		add_action( 'init', $bma_simple_card_boot, 20 );
	}
} else {
