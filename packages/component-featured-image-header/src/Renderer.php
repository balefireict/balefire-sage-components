<?php
/**
 * Renderer — delegates to the Blade view for Featured Image Header.
 *
...
namespace BalefireInc\Sage\FeaturedImageHeader;
...
class Renderer {
	...
	public static function render( array $props, string $wrapper_attributes = '' ): string {
		...
		if ( function_exists( '\Roots\view' ) ) {
			return \Roots\view( 'bma::components.featured-image-header', $props )->render();
		}

		// Acorn installed standalone.
		if ( function_exists( '\Acorn\view' ) ) {
			return \Acorn\view( 'bma::components.featured-image-header', $props )->render();
		}

		// No Blade runtime available — fail loud, not silent.
		return '<!-- balefire/featured-image-header: Sage/Acorn Blade runtime not found. '
