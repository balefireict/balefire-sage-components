<?php
/**
 * balefireict/component-section — bootstrap.
 *
 ...
 * Register the block.
 */
$bma_section_boot = static function (): void {

	// Honor the wp-admin "Balefire Blocks" toggle for this block.
	if ( ! \BalefireInc\Sage\Support\Settings::isBlockEnabled( 'section' ) ) {
		return;
	}
	...
	if ( is_admin() && function_exists( 'wp_register_script' ) ) {
		...
		$editor_js = file_get_contents( __DIR__ . '/../blocks/section/editor.js' );
		if ( $editor_js !== false ) {
			wp_add_inline_script( 'balefire-section-editor', $editor_js );
		}
	}
...
if ( function_exists( 'add_action' ) ) {
	if ( did_action( 'init' ) ) {
		...
		add_action( 'init', $bma_section_boot, 20 );
	}
} else {
	...
	$GLOBALS['wp_filter']['init'][20][] = [
		'function'      => $bma_section_boot,
		'accepted_args' => 1,
	];
}

unset( $bma_section_boot );
