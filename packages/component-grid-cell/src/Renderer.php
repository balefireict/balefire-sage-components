<?php
/**
 * Renderer — delegates to the Blade view for Grid Cell.
 *
...
namespace BalefireInc\Sage\GridCell;
...
class Renderer {
	...
	public static function render( array $props, string $wrapper_attributes = '' ): string {
		...
		if ( function_exists( '\Roots\view' ) ) {
			return \Roots\view( 'bma::components.grid-cell', $props )->render();
		}

		// Acorn installed standalone.
		if ( function_exists( '\Acorn\view' ) ) {
			return \Acorn\view( 'bma::components.grid-cell', $props )->render();
		}

		// No Blade runtime available — fail loud, not silent.
		return '<!-- balefire/grid-cell: Sage/Acorn Blade runtime not found. '
