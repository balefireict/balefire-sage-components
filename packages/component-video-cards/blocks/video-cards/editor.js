...
const metadata = {
    ...
    "apiVersion": 3,
    "name": "balefire/video-cards",
    "title": "Video Cards",
    "category": "balefire",
    "icon": "video-alt3",
    "description": "Grid of media cards with play overlay, badge, title, and blurb.",
    "keywords": [
        "video",
        "episodes",
        "media",
    ...
    "editorScript": "balefire-video-cards-editor"
};
                ...
                el(PanelBody, { title: 'Items', initialOpen: true },
                    ...items.map((item, index) =>
                        el('div', { key: index, style: { border: '1px solid #ddd', borderRadius: '4px', padding: '12px', marginBottom: '12px' } },
                            el(TextControl, { label: 'Badge', value: item.badge || '', onChange: (v) => updateItem(index, { badge: v }) }), el(TextControl, { label: 'Title', value: item.title || '', onChange: (v) => updateItem(index, { title: v }) }), el(TextareaControl, { label: 'Text', value: item.text || '', onChange: (v) => updateItem(index, { text: v }) }), el(TextControl, { label: 'ImageUrl', value: item.imageUrl || '', onChange: (v) => updateItem(index, { imageUrl: v }) }), el(TextControl, { label: 'ImageId', value: item.imageId || '', onChange: (v) => updateItem(index, { imageId: parseInt(v, 10) || 0 }) }), el(TextControl, { label: 'Url', value: item.url || '', onChange: (v) => updateItem(index, { url: v }) }),
                    ...
                    el(Button, { variant: 'primary', onClick: () => setAttributes({ items: [...items, { badge: '', title: '', text: '', imageUrl: '', imageId: 0, url: '' }] }) }, 'Add Item')
                )
            ),
            el('div', blockProps,
                el('p', { style: { margin: 0, fontFamily: 'monospace', fontSize: '11px', textTransform: 'uppercase', color: '#d72b27', fontWeight: 700 } }, 'Video Cards'),
