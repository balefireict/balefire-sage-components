<?php
/**
 * balefireict/component-video-cards — bootstrap.
 *
 ...
 *
 * @package BalefireInc\Sage\VideoCards
 */
 ...
 * Register the block.
 */
$bma_video_cards_boot = static function (): void {
	...
	if ( is_admin() && function_exists( 'wp_register_script' ) ) {
		...
		$editor_js = file_get_contents( __DIR__ . '/../blocks/video-cards/editor.js' );
		if ( $editor_js !== false ) {
			wp_add_inline_script( 'balefire-video-cards-editor', $editor_js );
		}
	}

	// --- Gutenberg block ---------------------------------------------------
	if ( function_exists( 'register_block_type' ) ) {
		register_block_type( __DIR__ . '/../blocks/video-cards' );
	}
};

if ( function_exists( 'add_action' ) ) {
	if ( did_action( 'init' ) ) {
		$bma_video_cards_boot();
	} else {
		add_action( 'init', $bma_video_cards_boot, 20 );
	}
} else {
	...
	$GLOBALS['wp_filter']['init'][20][] = [
		'function'      => $bma_video_cards_boot,
		'accepted_args' => 1,
	];
}

unset( $bma_video_cards_boot );
