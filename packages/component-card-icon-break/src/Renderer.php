<?php
/**
 * Renderer — delegates to the Blade view for Card Icon Break.
 *
...
namespace BalefireInc\Sage\CardIconBreak;
...
class Renderer {
	...
	public static function render( array $props, string $wrapper_attributes = '' ): string {
		...
		if ( function_exists( '\Roots\view' ) ) {
			return \Roots\view( 'bma::components.card-icon-break', $props )->render();
		}

		// Acorn installed standalone.
		if ( function_exists( '\Acorn\view' ) ) {
			return \Acorn\view( 'bma::components.card-icon-break', $props )->render();
		}

		// No Blade runtime available — fail loud, not silent.
		return '<!-- balefire/card-icon-break: Sage/Acorn Blade runtime not found. '
