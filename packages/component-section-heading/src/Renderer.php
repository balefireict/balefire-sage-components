<?php
/**
 * Renderer — delegates to the Blade view for Section Heading.
 *
...
namespace BalefireInc\Sage\SectionHeading;
...
class Renderer {
	...
	public static function render( array $props, string $wrapper_attributes = '' ): string {
		...
		if ( function_exists( '\Roots\view' ) ) {
			return \Roots\view( 'bma::components.section-heading', $props )->render();
		}

		// Acorn installed standalone.
		if ( function_exists( '\Acorn\view' ) ) {
			return \Acorn\view( 'bma::components.section-heading', $props )->render();
		}

		// No Blade runtime available — fail loud, not silent.
		return '<!-- balefire/section-heading: Sage/Acorn Blade runtime not found. '
