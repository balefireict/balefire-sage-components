<?php
/**
 * balefireict/component-info-cards — bootstrap.
 *
 ...
 * Register the block.
 */
$bma_info_cards_boot = static function (): void {
	...
	if ( is_admin() && function_exists( 'wp_register_script' ) ) {
		...
		$editor_js = file_get_contents( __DIR__ . '/../blocks/info-cards/editor.js' );
		if ( $editor_js !== false ) {
			wp_add_inline_script( 'balefire-info-cards-editor', $editor_js );
		}
	}

	// --- Gutenberg block ---------------------------------------------------
	if ( function_exists( 'register_block_type' ) ) {
		register_block_type( __DIR__ . '/../blocks/info-cards' );
	}
};
...
if ( function_exists( 'add_action' ) ) {
	if ( did_action( 'init' ) ) {
		...
		add_action( 'init', $bma_info_cards_boot, 20 );
	}
} else {
	...
	$GLOBALS['wp_filter']['init'][20][] = [
		'function'      => $bma_info_cards_boot,
		'accepted_args' => 1,
	];
}

unset( $bma_info_cards_boot );
